...
LRESULT CALLBACK MouseHookProc(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode == HC_ACTION) {
        ...
        if (hwndGlobal && GetWindowRect(hwndGlobal, &rect)) {
            // La barra principale della batteria occupa esattamente i 40 pixel in basso (sulla taskbar)
            ...
            else if ((wParam == WM_LBUTTONDOWN || wParam == WM_LBUTTONUP) && PtInRect(&mainBarRect, pt)) {
                if (!isMenuOpen) {
            ...
            else if ((wParam == WM_RBUTTONDOWN || wParam == WM_RBUTTONUP) && PtInRect(&mainBarRect, pt)) {
                if (!isMenuOpen) {
                    if (wParam == WM_RBUTTONUP) {
                        PostMessage(hwndGlobal, WM_USER_SHOW_MENU, pt.x, pt.y);
                    }
                    return 1; // Intercetta il clic (SIA PRESSIONE CHE RILASCIO)
                }
            }
